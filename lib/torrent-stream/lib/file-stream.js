...
var FileStream = function (engine, file, opts) {
  ...
  clivas.clear()
  clivas.line('{yellow:file read:} ' + 'start:' + this.num2s(opts.start) + ' end:' + this.num2s(opts.end)  + ' len:' + this.num2s(this.length))
  clivas.line('')
...
FileStream.prototype.num2s = function (num) {
  if (num / (1024 * 1024 * 1024) | 0 > 0 ) {
    var ret = num / (1024 * 1024 * 1024) | 0
    return  ret + 'G'
  } else if (num / (1024 * 1024) | 0 > 0 ) {
    var ret = num / (1024 * 1024) | 0
    return  ret + 'M'
  } else if (num / (1024) | 0 > 0 ) {
    var ret = num / (1024) | 0
    return  ret + 'Kb'
  } else {
    return num + 'b'
